...
function declareTributes(tributeArr){
    let newArr = tributeArr.map((element) => {
        return `${element.name} from district ${element.district}`
    })
    console.log(newArr)
}
...
function removeFaultyToys(arr){
    let qualityToys = arr.filter((element) => {
        return !element.containsChemicals
    })
    console.log(qualityToys)
}
